...
// The two read mechanisms are not compatible, so we must choose them at initialization.
...
#else
    fl_QuadDeviceSpec qspi_spec[RESPEAKER_LITE_QSPI_SPEC_COUNT] = {FL_QUADDEVICE_W25Q32JV, FL_QUADDEVICE_W25Q64JW, FL_QUADDEVICE_ZB25VQ32D};
    ...
    xassert(fl_connectToDevice(&qspi_ports, qspi_spec, RESPEAKER_LITE_QSPI_SPEC_COUNT) == 0);
    fl_setBootPartitionSize(RESPEAKER_LITE_BOOT_PARTITION_SIZE);

    rtos_dfu_image_init(
            dfu_image_ctx,
            &qspi_ports,
            qspi_spec,
            RESPEAKER_LITE_QSPI_SPEC_COUNT);
